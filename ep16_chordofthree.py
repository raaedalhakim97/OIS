...
def build_audio():
    ...
    # three — THE SONG: the canon, voiced properly, going round twice
    say(d, "together", 99.6, style="confident", **SH)
    for i, (t0, nm) in enumerate(SONG):
        dur = 3.0 if i < 8 else 1.75
        amp = 0.85 + 0.16 * (i / 8.0) + (0.30 if i >= 8 else 0.0)     # it grows, then runs hot
        sb.chord(d, w, nm, t0, dur=dur * 1.08, amp=amp, pan=0.46,
                 roll=0.030 if i < 8 else 0.018, padamp=0.040)
    ...
    sb.sing(d, sb.THEME, 124.2, beat=0.44, amp=0.105, pan=0.56, base=72)
