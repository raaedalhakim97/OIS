...
BLINKS = [(38.6, 0.24, 0.08, 0.38, 1.0),
          ...
          (138.8, 0.10, 0.03, 0.16, 0.55),    # the song ruptures
          (147.2, 0.20, 0.10, 0.40, 0.9),     # the Elder goes out
          (166.0, 0.85, 0.30, 1.10, 1.0)]
...
# THE SONG — three voices, and they do not hold one chord: they play a progression.
# Pachelbel's canon loop (public domain), in C: the most inevitable, most homeward
# sequence in music. It goes round twice — the second time faster, with the theme over it.
CANON = ["C", "G", "Am", "Em", "F", "C", "F", "G"]
CH_DEG = {"C": [0, 2, 4], "G": [4, 6, 1], "Am": [5, 7, 2], "Em": [2, 4, 6], "F": [3, 5, 7]}
CHORD0 = 100.0
LOOP1 = [(CHORD0 + i * 3.0, nm) for i, nm in enumerate(CANON)]            # 100 -> 124
LOOP2 = [(124.0 + i * 1.75, nm) for i, nm in enumerate(CANON)]           # 124 -> 138, the climax
SONG = LOOP1 + LOOP2
CHORD_HITS = [b for b in BEATS if 106 <= b <= 136][::2]   # he strikes it, and it holds
TAKE = 139.0                                          # he takes the Elder
OUT = 147.0                                           # the Elder's light goes out


def song_at(ts):
    for i, (t0, nm) in enumerate(SONG):
        dur = 3.0 if i < 8 else 1.75
        if t0 <= ts <= t0 + dur:
            return (t0, nm, CH_DEG[nm], (ts - t0) / dur, i)
    return None
...
def sunderer_x(ts):
    return lerp(SUN_X0, SUN_X1, smooth(1.0, 9.0, ts)) + 26 * smooth(133, 137, ts)
...
CAPS = [
    ...
    (99.6, 101.2, "together."),
    (102.0, 104.4, "so they sounded three."),
    (105.2, 108.8, "and they did not\nhold one chord."),
    (109.6, 112.0, "they played a song."),
    (115.0, 119.0, "he could copy a note.\nhe could copy the beat."),
    (120.0, 122.4, "but a chord is not a sound."),
    (123.0, 127.0, "it is what happens\nbetween more than one."),
    (128.2, 130.4, "and he was alone."),
    (131.4, 134.0, "so the song went round again —"),
    (134.6, 137.4, "and it came home."),
    (139.4, 142.0, "so it did not break the chord."),
    (142.8, 145.2, "it broke the three."),
    (148.6, 150.4, "listen."),
    (155.0, 158.0, "and then there were two."),
    (158.6, 160.4, "no."),
]
...
def end_card(im, ts):
    if ts < 164.0: return
    d = ImageDraw.Draw(im, "RGBA")
    fade = np.interp(ts, [164.0, 165.2, 167.4, 169.0], [0, 1, 1, 0])
...
def story(ts):
    ...
    # ---- three: THE SONG (a progression, not one held chord) ----
    ch = smooth(CHORD0, CHORD0 + 1.4, ts) * (1 - smooth(TAKE, TAKE + 1.0, ts))
    sg = song_at(ts)
    if ch > 0.02:
        col = GOLD
        if sg is not None:
            col = VIOLET if sg[1].endswith("m") else GOLD
            u = sg[3]
            pulse = 0.55 + 0.45 * math.exp(-u * 3.4)          # each chord lands and blooms
        else:
            pulse = 0.5
        hit = 0.0
        for t0 in CHORD_HITS:
            if 0 <= ts - t0 <= 0.7: hit = max(hit, 1 - (ts - t0) / 0.7)
        drive = 1.0 + 0.35 * smooth(124, 138, ts)             # the second loop runs hotter
        amp = ch * (0.34 + 0.26 * hit) * pulse * drive
        thread(a, KP, EP_, amp, col, n=18)                     # the triangle, in the chord's colour
        thread(a, EP_, SP, amp, col, n=18)
        thread(a, SP, KP, amp, LAV if sg and sg[1].endswith("m") else WARM, n=18)
        cxm = (KP[0] + EP_[0] + SP[0]) / 3; cym = (KP[1] + EP_[1] + SP[1]) / 3
        glow(a, cxm, cym, 38 + 44 * pulse * drive, col, ch * (0.20 + 0.26 * pulse))
        thread(a, (cxm, cym), (sx - 0.045 * W, sy - 0.06 * H), amp * 0.6, col, n=14, w=7)
        if sg is not None:                                     # the chord's notes light the staff
            for g in sg[2]:
                glow(a, SLOT[g][0], SLOT[g][1], 15 + 9 * pulse, col, 0.35 + 0.5 * pulse)
    ...
    sgn = song_at(ts)
    if sgn is not None:                                   # the chord we are on
        t0, nm, dg, u, idx = sgn
        fade = float(np.interp(u, [0, 0.12, 0.75, 0.95], [0, 1, 1, 0]))
        col = (206, 190, 244) if nm.endswith("m") else (255, 214, 150)
        d4 = ImageDraw.Draw(im, "RGBA")
        lbl = nm + "   " + " ".join(["Do", "Re", "Mi", "Fa", "Sol", "La", "Ti", "Do"][g] for g in dg)
        bb = d4.textbbox((0, 0), lbl, font=FSM); xx = (W - (bb[2] - bb[0])) // 2
        d4.text((xx, int(SY + 5.3 * LG)), lbl, font=FSM, fill=col + (int(230 * fade),))
    pwn = power_at(ts)
...
def build_audio():
    ...
    w(pad([midi(44), midi(51), midi(56), midi(59)], 46, 0.04), 1, 0.5)
    ...
    # three — THE SONG: the canon, properly voiced, going round twice
    say(d, "together", 99.6, style="confident", **SH)
    BASSNOTE = {"C": 36, "G": 31, "Am": 33, "Em": 28, "F": 29}
    for i, (t0, nm) in enumerate(SONG):
        dur = 3.0 if i < 8 else 1.75
        amp = 0.88 + 0.14 * (i / 8.0) + (0.32 if i >= 8 else 0.0)      # grows, then runs hot
        sb.chord(d, w, nm, t0, dur=dur * 1.1, amp=amp, pan=0.46,
                 roll=0.030 if i < 8 else 0.017, padamp=0.042)
        w(bass(midi(BASSNOTE[nm]), dur * 1.15, 0.055), t0, 0.5)        # the walking bass line
        d(softkick(0.085 if i < 8 else 0.115), t0, 0.5)
    # the theme, sung over the second time round — the climax
    sb.sing(d, sb.THEME, 124.2, beat=0.44, amp=0.105, pan=0.58, base=72)
    for t0 in CHORD_HITS:                                              # his blows, absorbed
        d(softkick(0.09), t0, 0.62); w(bass(midi(29), 1.0, 0.04), t0, 0.64)

    # he takes the Elder
    # the song is one chord from home — and it is broken off. a deceptive, sickening turn.
    sb.chord(d, w, "Ab", TAKE, dur=5.0, amp=1.15, pan=0.5, roll=0.012, padamp=0.055)
    d(softkick(0.19), TAKE, 0.5); w(bass(midi(26), 12, 0.07), TAKE, 0.55)
    d(piano(midi(61), 3.0, 0.10), TAKE + 0.06, 0.62)               # a note from outside the key
    for i, m in enumerate([67, 65, 63, 61, 59, 57]):               # the Elder's light sliding away
        d(piano(midi(m), 2.4, 0.085 - 0.010 * i), TAKE + 1.2 + i * 0.75, 0.48)
    say(d, "listen", 146.6, style="whisper", register=-12, pan=0.46)
    d(piano(midi(48), 7.0, 0.10), OUT, 0.46)
    d(softkick(0.17), OUT, 0.5); w(bass(midi(26), 14, 0.06), OUT, 0.5)
    w(pad([midi(44), midi(51), midi(56)], 10, 0.045), OUT + 0.4, 0.5)
    # and then there were two
    for t in np.arange(155.0, 163, 3.6):                          # a bare fifth: only two left
        d(piano(midi(60), 3.4, 0.105), t, 0.34); d(piano(midi(67), 3.4, 0.085), t + 0.02, 0.58)
        w(bass(midi(36), 3.6, 0.045), t, 0.5)
    sb.sing(d, sb.THEME[:6] + [(None, 2)], 156.0, beat=0.62, amp=0.085, pan=0.44, swell=False)
    say(d, "no", 158.6, style="urgent", **K)
    w(pad([midi(44), midi(51), midi(56)], 14, 0.04), 155, 0.5)
